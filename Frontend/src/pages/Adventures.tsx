...
const Adventures = () => {
  ...
  const handleAdventureCreated = async (storyData: {
    ...
    const newAdventure: Adventure = {
      ...
      // CRITICAL FIX: Don't show loading state immediately
      isImagesLoading: false, // Changed from imageGenerationEnabled
      imageLoadError: false,
    };
    ...
    // CRITICAL FIX: Delay image loading to avoid immediate CORS errors
    if (imageGenerationEnabled) {
      console.log("🖼️ Starting delayed image loading for new adventure");

      // Wait a bit before starting image loading
      setTimeout(() => {
        // Update to show loading state only after delay
        setAdventures((prev) =>
          prev.map((adv) =>
            adv.sessionId === storyData.sessionId
              ? { ...adv, isImagesLoading: true }
              : adv
          )
        );

        // Start waiting for images with longer delay
        waitForNewAdventureImages(storyData.sessionId);
      }, 3000); // 3 second delay to let backend process
    }
  };
                        ...
                        <div className="flex-shrink-0 relative">
                          ...
                          {imageGenerationEnabled && adventure.imageStatus && (
                            <div className="absolute -bottom-1 -right-1">
                              {adventure.isImagesLoading ? (
                                <div
                                  className="w-3 h-3 bg-yellow-500 rounded-full border border-background animate-pulse"
                                  title="Generating images..."
                                />
                              ) : adventure.imageLoadError ? (
                                <div
                                  className="w-3 h-3 bg-red-500 rounded-full border border-background"
